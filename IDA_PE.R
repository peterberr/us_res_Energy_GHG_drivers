...
r$HeatFuel[r$HeatFuel==2 | r$HeatFuel == 6 | r$HeatFuel==7 | r$HeatFuel== 8 | r$HeatFuel ==9]<-"21" # add fuels to 'other'
r$HeatFuel<-droplevels(r$HeatFuel)
r$DHWFuel<-r$FUELH2O
r$DHWFuel[r$DHWFuel==2 | r$DHWFuel == 6 | r$DHWFuel==7 | r$DHWFuel== 8 | r$DHWFuel ==9]<-"21" # add fuels to 'other'
...
E190<-tapply(r90$PEstSPHwa*r90$NWEIGHT,list(r90$TYPEHUQ,r90$DIVISION,r90$AgeCohort,r90$HeatFuel),sum)  # energy for space heat in 1990 by type, division, agecohort, main fuel type
E190[is.na(E190)]<-0
...
I190[is.nan(I190)]<-0
E190_o<-tapply(r90$PESPH*r90$NWEIGHT,list(r90$TYPEHUQ,r90$DIVISION,r90$AgeCohort,r90$HeatFuel),sum)  # energy for space heat in 1990 by type, division, agecohort
E190_o[is.na(E190_o)]<-0
E190_w<-tapply(r90$PESPHwa*r90$NWEIGHT,list(r90$TYPEHUQ,r90$DIVISION,r90$AgeCohort,r90$HeatFuel),sum) 
...
E193<-tapply(r93$PEstSPHwa*r93$NWEIGHT,list(r93$TYPEHUQ,r93$DIVISION,r93$AgeCohort,r93$HeatFuel),sum)  # energy for space heat in 1993 by type, division, agecohort, main fuel type
E193[is.na(E193)]<-0
...
I193[is.nan(I193)]<-0
E193_o<-tapply(r93$PESPH*r93$NWEIGHT,list(r93$TYPEHUQ,r93$DIVISION,r93$AgeCohort,r93$HeatFuel),sum)  # energy for space heat in 1993 by type, division, agecohort
E193_o[is.na(E193_o)]<-0
E193_w<-tapply(r93$PESPHwa*r93$NWEIGHT,list(r93$TYPEHUQ,r93$DIVISION,r93$AgeCohort,r93$HeatFuel),sum)  # energy for space heat in 1993 by type, division, agecohort
...
E101<-tapply(r01$PEstSPHwa*r01$NWEIGHT,list(r01$TYPEHUQ,r01$DIVISION,r01$AgeCohort,r01$HeatFuel),sum)  # energy for space heat in 1901 by type, division, agecohort, main fuel type
E101[is.na(E101)]<-0
...
E101_o<-tapply(r01$PESPH*r01$NWEIGHT,list(r01$TYPEHUQ,r01$DIVISION,r01$AgeCohort,r01$HeatFuel),sum)  # energy for space heat in 2001 by type, division, agecohort
E101_o[is.na(E101_o)]<-0
E101_w<-tapply(r01$PESPHwa*r01$NWEIGHT,list(r01$TYPEHUQ,r01$DIVISION,r01$AgeCohort,r01$HeatFuel),sum) 
...
# decomp new sph 2005 ######
E105<-tapply(r05$PEstSPHwa*r05$NWEIGHT,list(r05$TYPEHUQ,r05$DIVISION,r05$AgeCohort,r05$HeatFuel),sum)  # energy for space heat in 1905 by type, division, agecohort, main fuel type
E105[is.na(E105)]<-0
...
E105_o<-tapply(r05$PESPH*r05$NWEIGHT,list(r05$TYPEHUQ,r05$DIVISION,r05$AgeCohort,r05$HeatFuel),sum)  # energy for space heat in 2005 by type, division, agecohort
E105_o[is.na(E105_o)]<-0
E105_w<-tapply(r05$PESPHwa*r05$NWEIGHT,list(r05$TYPEHUQ,r05$DIVISION,r05$AgeCohort,r05$HeatFuel),sum) 
...
E109<-tapply(r09$PEstSPHwa*r09$NWEIGHT,list(r09$TYPEHUQ,r09$DIVISION,r09$AgeCohort,r09$HeatFuel),sum)  # energy for space heat in 2009 by type, division, agecohort, main fuel type
E109[is.na(E109)]<-0
...
E109_o<-tapply(r09$PESPH*r09$NWEIGHT,list(r09$TYPEHUQ,r09$DIVISION,r09$AgeCohort,r09$HeatFuel),sum)  # energy for space heat in 2009 by type, division, agecohort
E109_o[is.na(E109_o)]<-0
E109_w<-tapply(r09$PESPHwa*r09$NWEIGHT,list(r09$TYPEHUQ,r09$DIVISION,r09$AgeCohort,r09$HeatFuel),sum) 
...
E115<-tapply(r15$PEstSPHwa*r15$NWEIGHT,list(r15$TYPEHUQ,r15$DIVISION,r15$AgeCohort,r15$HeatFuel),sum)  # energy for space heat in 1915 by type, division, agecohort, main fuel type
E115[is.na(E115)]<-0
...
E115_o<-tapply(r15$PESPH*r15$NWEIGHT,list(r15$TYPEHUQ,r15$DIVISION,r15$AgeCohort,r15$HeatFuel),sum)  # energy for space heat in 2015 by type, division, agecohort
E115_o[is.na(E115_o)]<-0
E115_w<-tapply(r15$PESPHwa*r15$NWEIGHT,list(r15$TYPEHUQ,r15$DIVISION,r15$AgeCohort,r15$HeatFuel),sum) 
...
hpc9093<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Occupancy","Conditioned Space","Intensity","Electricity Efficiency", "Weather"),
...
habs9093<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
...
sum_eff1<-delP1+delR1+delT1+delC1+delH1+delO1+delL1+delI1+delX1+delW1
db<-sum_eff1-diff1 # for debugging
hpc9301<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
                    pc=100*c(deltotpc1,dPpcbas1,dRpcbas1,dTpcbas1,dCpcbas1,dHpcbas1,dOpcbas1,dLpcbas1,dIpcbas1,dXpcbas1,dWpcbas1))
#Turn your 'treatment' column into a character vector
...
ggplot(data=hpc9301, aes(x=Effect, y=pc)) +
  ...
  labs(title = "Percent changes in space heating 1993-2001", x= "Effect", 
       y= "%",caption = "Model 3") + theme(axis.text=element_text(size=10.5),
                                           axis.title=element_text(size=12,face = "bold"),
                                           plot.title = element_text(size = 14, face = "bold"))
habs9301<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
                     abs=c(deltot1,delPtot1,delRtot1,delTtot1,delCtot1,delHtot1,delOtot1,delLtot1,delItot1,delXtot1,delWtot1))
# check
...
sum_eff1<-delP1+delR1+delT1+delC1+delH1+delO1+delL1+delI1+delX1+delW1
db<-sum_eff1-diff1 # for debugging
hpc0105<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
                    pc=100*c(deltotpc1,dPpcbas1,dRpcbas1,dTpcbas1,dCpcbas1,dHpcbas1,dOpcbas1,dLpcbas1,dIpcbas1,dXpcbas1,dWpcbas1))
#Turn your 'treatment' column into a character vector
...
ggplot(data=hpc0105, aes(x=Effect, y=pc)) +
  ...
  labs(title = "Percent changes in space heating 2001-2005", x= "Effect", 
       y= "%",caption = "Model 3") + theme(axis.text=element_text(size=10.5),
                                           axis.title=element_text(size=12,face = "bold"),
                                           plot.title = element_text(size = 14, face = "bold"))
habs0105<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
                     abs=c(deltot1,delPtot1,delRtot1,delTtot1,delCtot1,delHtot1,delOtot1,delLtot1,delItot1,delXtot1,delWtot1))
# check
...
sum_eff1<-delP1+delR1+delT1+delC1+delH1+delO1+delL1+delI1+delX1+delW1
db<-sum_eff1-diff1 # for debugging
hpc0509<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
                    pc=100*c(deltotpc1,dPpcbas1,dRpcbas1,dTpcbas1,dCpcbas1,dHpcbas1,dOpcbas1,dLpcbas1,dIpcbas1,dXpcbas1,dWpcbas1))
#Turn your 'treatment' column into a character vector
...
ggplot(data=hpc0509, aes(x=Effect, y=pc)) +
  ...
  labs(title = "Percent changes in space heating 2005-2009", x= "Effect", 
       y= "%",caption = "Model 3") + theme(axis.text=element_text(size=10.5),
                                           axis.title=element_text(size=12,face = "bold"),
                                           plot.title = element_text(size = 14, face = "bold"))
habs0509<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
                     abs=c(deltot1,delPtot1,delRtot1,delTtot1,delCtot1,delHtot1,delOtot1,delLtot1,delItot1,delXtot1,delWtot1))
# check
habs0509$abs[1]-sum(habs0509$abs[2:11])
...
sum_eff1<-delP1+delR1+delT1+delC1+delH1+delO1+delL1+delI1+delX1+delW1
db<-sum_eff1-diff1 # for debugging
hpc0915<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
                    pc=100*c(deltotpc1,dPpcbas1,dRpcbas1,dTpcbas1,dCpcbas1,dHpcbas1,dOpcbas1,dLpcbas1,dIpcbas1,dXpcbas1,dWpcbas1))
#Turn your 'treatment' column into a character vector
...
ggplot(data=hpc0915, aes(x=Effect, y=pc)) +
  ...
  labs(title = "Percent changes in space heating 2009-2015", x= "Effect", 
       y= "%",caption = "Model 3") + theme(axis.text=element_text(size=10.5),
                                           axis.title=element_text(size=12,face = "bold"),
                                           plot.title = element_text(size = 14, face = "bold"))
habs0915<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
                     abs=c(deltot1,delPtot1,delRtot1,delTtot1,delCtot1,delHtot1,delOtot1,delLtot1,delItot1,delXtot1,delWtot1))
# check
...
eff<-c("Population","Region","Type","Cohort","Fuel","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather")
habsall<-data.frame(Effect=c("1990",eff,"1993",eff,"2001",eff,"2005",eff,"2009",eff,"2015"),
                    kBTU=c(sum(e190),habs9093$abs[2:11],sum(e193),habs9301$abs[2:11],sum(e101),habs0105$abs[2:11],sum(e105),habs0509$abs[2:11],sum(e109),habs0915$abs[2:11],sum(e115)))
habsall$PJ<-1.055e-9*habsall$kBTU
habsall$Effect3<-c("1990",eff,"2001",eff,"2009",eff,"2015",rep("NA",22))
habsall$kBTU3<-c(sum(e190),habs909301$abs[2:11],sum(e101),habs010509$abs[2:11],sum(e109),habs0915$abs[2:11],sum(e115),rep(0,22)) 
habsall$PJ3<-1.055e-9*habsall$kBTU3
habsall$Effect1<-c("1990",eff,"2015",rep("NA",44))
habsall$kBTU1<-c(sum(e190),habs9015tot$abs[2:11],sum(e115),rep(0,44))
habsall$PJ1<-1.055e-9*habsall$kBTU1

res<-paste("Results", as.character(Sys.Date()))
dir.create(res)
write.csv(habsall,file = paste(res, "/sph_IDA_PE.csv",sep = ""))

####### IDA for space cooling ######

# cooling decomposition, 1990 ####
E290<-tapply(r90$PEstCOLwa*r90$NWEIGHT,list(r90$TYPEHUQ,r90$DIVISION,r90$AgeCohort),sum)  # adjusted energy for space cooling in 1990 by type, division, agecohort
E290[is.na(E290)]<-0
...
E290_o<-tapply(r90$PECOL*r90$NWEIGHT,list(r90$TYPEHUQ,r90$DIVISION,r90$AgeCohort),sum)  # energy for space cooling in 1990 by type, division, agecohort
E290_o[is.na(E290_o)]<-0
E290_w<-tapply(r90$PECOLwa*r90$NWEIGHT,list(r90$TYPEHUQ,r90$DIVISION,r90$AgeCohort),sum) 
...
E293<-tapply(r93$PEstCOLwa*r93$NWEIGHT,list(r93$TYPEHUQ,r93$DIVISION,r93$AgeCohort),sum)  # adjusted energy for space cooling in 1993 by type, division, agecohort
E293[is.na(E293)]<-0
...
I293[is.nan(I293)]<-0
E293_o<-tapply(r93$PECOL*r93$NWEIGHT,list(r93$TYPEHUQ,r93$DIVISION,r93$AgeCohort),sum)  # energy for space heat in 1993 by type, division, agecohort
E293_o[is.na(E293_o)]<-0
E293_w<-tapply(r93$PECOLwa*r93$NWEIGHT,list(r93$TYPEHUQ,r93$DIVISION,r93$AgeCohort),sum) 
...
E201<-tapply(r01$PEstCOLwa*r01$NWEIGHT,list(r01$TYPEHUQ,r01$DIVISION,r01$AgeCohort),sum)  # adjusted energy for space cooling in 2001 by type, division, agecohort
E201[is.na(E201)]<-0
...
I201[is.nan(I201)]<-0
E201_o<-tapply(r01$PECOL*r01$NWEIGHT,list(r01$TYPEHUQ,r01$DIVISION,r01$AgeCohort),sum)  # energy for space cooling in 2001 by type, division, agecohort
E201_o[is.na(E201_o)]<-0
E201_w<-tapply(r01$PECOLwa*r01$NWEIGHT,list(r01$TYPEHUQ,r01$DIVISION,r01$AgeCohort),sum) 
...
E205<-tapply(r05$PEstCOLwa*r05$NWEIGHT,list(r05$TYPEHUQ,r05$DIVISION,r05$AgeCohort),sum)  # adjusted energy for space cooling in 2005 by type, division, agecohort
E205[is.na(E205)]<-0
...
I205[is.nan(I205)]<-0
E205_o<-tapply(r05$PECOL*r05$NWEIGHT,list(r05$TYPEHUQ,r05$DIVISION,r05$AgeCohort),sum)  # energy for space cooling in 2005 by type, division, agecohort
E205_o[is.na(E205_o)]<-0
E205_w<-tapply(r05$PECOLwa*r05$NWEIGHT,list(r05$TYPEHUQ,r05$DIVISION,r05$AgeCohort),sum) 
...
E209<-tapply(r09$PEstCOLwa*r09$NWEIGHT,list(r09$TYPEHUQ,r09$DIVISION,r09$AgeCohort),sum)  # adjusted energy for space cooling in 2009 by type, division, agecohort
E209[is.na(E209)]<-0
...
E209_o<-tapply(r09$PECOL*r09$NWEIGHT,list(r09$TYPEHUQ,r09$DIVISION,r09$AgeCohort),sum)  # energy for space cooling in 2009 by type, division, agecohort
E209_o[is.na(E209_o)]<-0
E209_w<-tapply(r09$PECOLwa*r09$NWEIGHT,list(r09$TYPEHUQ,r09$DIVISION,r09$AgeCohort),sum) 
...
E215<-tapply(r15$PEstCOLwa*r15$NWEIGHT,list(r15$TYPEHUQ,r15$DIVISION,r15$AgeCohort),sum)  # adjusted energy for space cooling in 2015 by type, division, agecohort
E215[is.na(E215)]<-0
...
I215[is.nan(I215)]<-0
E215_o<-tapply(r15$PECOL*r15$NWEIGHT,list(r15$TYPEHUQ,r15$DIVISION,r15$AgeCohort),sum)  # energy for space cooling in 2015 by type, division, agecohort
E215_o[is.na(E215_o)]<-0
E215_w<-tapply(r15$PECOLwa*r15$NWEIGHT,list(r15$TYPEHUQ,r15$DIVISION,r15$AgeCohort),sum) 
...
cpc9093<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
...
ggplot(data=cpc9093, aes(x=Effect, y=pc)) +
  ...
  labs(title = "Percent changes in space cooling 1990-1993", x= "Effect", 
       y= "%",caption = "Model 3") + theme(axis.text=element_text(size=10.5),
                                           axis.title=element_text(size=12,face = "bold"),
                                           plot.title = element_text(size = 14, face = "bold"))
cabs9093<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
...
diff2<-e201-e293
sum_eff2<-delP2+delR2+delT2+delC2+delO2+delL2+delI2+delW2+delX2
cpc9301<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
                    pc=100*c(deltotpc2,dPpcbas2,dRpcbas2,dTpcbas2,dCpcbas2,dOpcbas2,dLpcbas2,dIpcbas2,dXpcbas2,dWpcbas2))
#Turn your 'treatment' column into a character vector
...
ggplot(data=cpc9301, aes(x=Effect, y=pc)) +
  ...
  labs(title = "Percent changes in space cooling 1993-2001", x= "Effect", 
       y= "%",caption = "Model 3") + theme(axis.text=element_text(size=10.5),
                                           axis.title=element_text(size=12,face = "bold"),
                                           plot.title = element_text(size = 14, face = "bold"))
cabs9301<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
                     abs=c(deltot2,delPtot2,delRtot2,delTtot2,delCtot2,delOtot2,delLtot2,delItot2,delXtot2,delWtot2))
# check
...
diff2<-e205-e201
sum_eff2<-delP2+delR2+delT2+delC2+delO2+delL2+delI2+delW2+delX2
cpc0105<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
                    pc=100*c(deltotpc2,dPpcbas2,dRpcbas2,dTpcbas2,dCpcbas2,dOpcbas2,dLpcbas2,dIpcbas2,dXpcbas2,dWpcbas2))
#Turn your 'treatment' column into a character vector
...
ggplot(data=cpc0105, aes(x=Effect, y=pc)) +
  ...
  labs(title = "Percent changes in space cooling 2001-2005", x= "Effect", 
       y= "%",caption = "Model 3") + theme(axis.text=element_text(size=10.5),
                                           axis.title=element_text(size=12,face = "bold"),
                                           plot.title = element_text(size = 14, face = "bold"))
cabs0105<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
                     abs=c(deltot2,delPtot2,delRtot2,delTtot2,delCtot2,delOtot2,delLtot2,delItot2,delXtot2,delWtot2))
# check
...
diff2<-e209-e205
sum_eff2<-delP2+delR2+delT2+delC2+delO2+delL2+delI2+delW2+delX2
cpc0509<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
                    pc=100*c(deltotpc2,dPpcbas2,dRpcbas2,dTpcbas2,dCpcbas2,dOpcbas2,dLpcbas2,dIpcbas2,dXpcbas2,dWpcbas2))
#Turn your 'treatment' column into a character vector
...
ggplot(data=cpc0509, aes(x=Effect, y=pc)) +
  ...
  labs(title = "Percent changes in space cooling 2005-2009", x= "Effect", 
       y= "%",caption = "Model 3") + theme(axis.text=element_text(size=10.5),
                                           axis.title=element_text(size=12,face = "bold"),
                                           plot.title = element_text(size = 14, face = "bold"))
cabs0509<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
                     abs=c(deltot2,delPtot2,delRtot2,delTtot2,delCtot2,delOtot2,delLtot2,delItot2,delXtot2,delWtot2))
# check
...
diff2<-e215-e209
sum_eff2<-delP2+delR2+delT2+delC2+delO2+delL2+delI2+delW2+delX2
cpc0915<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
                    pc=100*c(deltotpc2,dPpcbas2,dRpcbas2,dTpcbas2,dCpcbas2,dOpcbas2,dLpcbas2,dIpcbas2,dXpcbas2,dWpcbas2))
#Turn your 'treatment' column into a character vector
...
ggplot(data=cpc0915, aes(x=Effect, y=pc)) +
  ...
  labs(title = "Percent changes in space cooling 2009-2015", x= "Effect", 
       y= "%",caption = "Model 3") + theme(axis.text=element_text(size=10.5),
                                           axis.title=element_text(size=12,face = "bold"),
                                           plot.title = element_text(size = 14, face = "bold"))
cabs0915<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather"),
                     abs=c(deltot2,delPtot2,delRtot2,delTtot2,delCtot2,delOtot2,delLtot2,delItot2,delXtot2,delWtot2))
# check
...
eff<-c("Population","Region","Type","Cohort","Occupancy","Conditioned Space","Intensity","Electricity Efficiency","Weather")
cabsall<-data.frame(Effect=c("1990",eff,"1993",eff,"2001",eff,"2005",eff,"2009",eff,"2015"),
                    kBTU=c(sum(e290),cabs9093$abs[2:10],sum(e293),cabs9301$abs[2:10],sum(201),cabs0105$abs[2:10],sum(e205),cabs0509$abs[2:10],sum(e209),cabs0915$abs[2:10],sum(e215)))
cabsall$PJ<-1.055e-9*cabsall$kBTU
cabsall$Effect3<-c("1990",eff,"2001",eff,"2009",eff,"2015",rep("NA",20))
cabsall$kBTU3<-c(sum(e290),cabs909301$abs[2:10],sum(e201),cabs010509$abs[2:10],sum(e209),cabs0915$abs[2:10],sum(e215),rep(0,20)) 
cabsall$PJ3<-1.055e-9*cabsall$kBTU3
cabsall$Effect1<-c("1990",eff,"2015",rep("NA",40))
cabsall$kBTU1<-c(sum(e290),cabs9015tot$abs[2:10],sum(e215),rep(0,40))
cabsall$PJ1<-1.055e-9*cabsall$kBTU1
write.csv(cabsall,file = paste(res, "/spc_IDA_PE.csv",sep = ""))

# decomposition new dhw, incl type and age cohort, 1990 #####
E390<-tapply(r90$PEstDHWwa*r90$NWEIGHT,list(r90$TYPEHUQ,r90$DIVISION,r90$AgeCohort,r90$DHWFuel),sum)  # energy for hot water in 3990 by type, division, agecohort, main fuel type
E390[is.na(E390)]<-0
...
E390_o<-tapply(r90$PEDHW*r90$NWEIGHT,list(r90$TYPEHUQ,r90$DIVISION,r90$AgeCohort,r90$DHWFuel),sum)  # energy for water heating in 1990 by type, division, agecohort
E390_o[is.na(E390_o)]<-0
E390_w<-tapply(r90$PEDHWwa*r90$NWEIGHT,list(r90$TYPEHUQ,r90$DIVISION,r90$AgeCohort,r90$DHWFuel),sum) 
...
E393<-tapply(r93$PEstDHWwa*r93$NWEIGHT,list(r93$TYPEHUQ,r93$DIVISION,r93$AgeCohort,r93$DHWFuel),sum)  # energy for hot water in 3993 by type, division, agecohort, main fuel type
E393[is.na(E393)]<-0
...
E393_o<-tapply(r93$PEDHW*r93$NWEIGHT,list(r93$TYPEHUQ,r93$DIVISION,r93$AgeCohort,r93$DHWFuel),sum)  # energy for water heating in 1993 by type, division, agecohort
E393_o[is.na(E393_o)]<-0
E393_w<-tapply(r93$PEDHWwa*r93$NWEIGHT,list(r93$TYPEHUQ,r93$DIVISION,r93$AgeCohort,r93$DHWFuel),sum) 
...
E301<-tapply(r01$PEstDHWwa*r01$NWEIGHT,list(r01$TYPEHUQ,r01$DIVISION,r01$AgeCohort,r01$DHWFuel),sum)  # energy for hot water in 3901 by type, division, agecohort, main fuel type
E301[is.na(E301)]<-0
...
E301_o<-tapply(r01$PEDHW*r01$NWEIGHT,list(r01$TYPEHUQ,r01$DIVISION,r01$AgeCohort,r01$DHWFuel),sum)  # energy for water heating in 2001 by type, division, agecohort
E301_o[is.na(E301_o)]<-0
E301_w<-tapply(r01$PEDHWwa*r01$NWEIGHT,list(r01$TYPEHUQ,r01$DIVISION,r01$AgeCohort,r01$DHWFuel),sum) 
...
E305<-tapply(r05$PEstDHWwa*r05$NWEIGHT,list(r05$TYPEHUQ,r05$DIVISION,r05$AgeCohort,r05$DHWFuel),sum)  # energy for hot water in 3905 by type, division, agecohort, main fuel type
E305[is.na(E305)]<-0
...
E305_o<-tapply(r05$PEDHW*r05$NWEIGHT,list(r05$TYPEHUQ,r05$DIVISION,r05$AgeCohort,r05$DHWFuel),sum)  # energy for water heating in 2005 by type, division, agecohort
E305_o[is.na(E305_o)]<-0
E305_w<-tapply(r05$PEDHWwa*r05$NWEIGHT,list(r05$TYPEHUQ,r05$DIVISION,r05$AgeCohort,r05$DHWFuel),sum) 
...
E309<-tapply(r09$PEstDHWwa*r09$NWEIGHT,list(r09$TYPEHUQ,r09$DIVISION,r09$AgeCohort,r09$DHWFuel),sum)  # energy for hot water in 3909 by type, division, agecohort, main fuel type
E309[is.na(E309)]<-0
...
E309_o<-tapply(r09$PEDHW*r09$NWEIGHT,list(r09$TYPEHUQ,r09$DIVISION,r09$AgeCohort,r09$DHWFuel),sum)  # energy for water heating in 2009 by type, division, agecohort
E309_o[is.na(E309_o)]<-0
E309_w<-tapply(r09$PEDHWwa*r09$NWEIGHT,list(r09$TYPEHUQ,r09$DIVISION,r09$AgeCohort,r09$DHWFuel),sum) 
...
E315<-tapply(r15$PEstDHWwa*r15$NWEIGHT,list(r15$TYPEHUQ,r15$DIVISION,r15$AgeCohort,r15$DHWFuel),sum)  # energy for hot water in 3915 by type, division, agecohort, main fuel type
E315[is.na(E315)]<-0
...
E315_o<-tapply(r15$PEDHW*r15$NWEIGHT,list(r15$TYPEHUQ,r15$DIVISION,r15$AgeCohort,r15$DHWFuel),sum)  # energy for water heating in 2015 by type, division, agecohort
E315_o[is.na(E315_o)]<-0
E315_w<-tapply(r15$PEDHWwa*r15$NWEIGHT,list(r15$TYPEHUQ,r15$DIVISION,r15$AgeCohort,r15$DHWFuel),sum) 
...
wpc9093<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Intensity","Electricity Efficiency","Weather"),
...
ggplot(data=wpc9093, aes(x=Effect, y=pc)) +
  ...
  labs(title = "Percent changes in domestic hot water 1990-1993", x= "Effect", 
       y= "%",caption = "Model 3") + theme(axis.text=element_text(size=10.5),
                                           axis.title=element_text(size=12,face = "bold"),
                                           plot.title = element_text(size = 14, face = "bold"))
wabs9093<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Intensity","Electricity Efficiency","Weather"),
...
sum_eff3<-delP3+delR3+delT3+delC3+delH3+delI3+delX3+delW3
db<-sum_eff3-diff3 # for debugging
wpc9301<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Intensity","Electricity Efficiency","Weather"),
                    pc=100*c(deltotpc3,dPpcbas3,dRpcbas3,dTpcbas3,dCpcbas3,dHpcbas3,dIpcbas3,dXpcbas3,dWpcbas3))
#Turn your 'treatment' column into a character vector
...
ggplot(data=wpc9301, aes(x=Effect, y=pc)) +
  ...
  labs(title = "Percent changes in domestic hot water 1993-2001", x= "Effect", 
       y= "%",caption = "Model 3") + theme(axis.text=element_text(size=10.5),
                                           axis.title=element_text(size=12,face = "bold"),
                                           plot.title = element_text(size = 14, face = "bold"))
wabs9301<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Intensity","Electricity Efficiency","Weather"),
                     abs=c(deltot3,delPtot3,delRtot3,delTtot3,delCtot3,delHtot3,delItot3,delXtot3,delWtot3))
# check
...
sum_eff3<-delP3+delR3+delT3+delC3+delH3+delI3+delX3+delW3
db<-sum_eff3-diff3 # for debugging
wpc0105<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Intensity","Electricity Efficiency","Weather"),
                    pc=100*c(deltotpc3,dPpcbas3,dRpcbas3,dTpcbas3,dCpcbas3,dHpcbas3,dIpcbas3,dXpcbas3,dWpcbas3))
#Turn your 'treatment' column into a character vector
...
ggplot(data=wpc0105, aes(x=Effect, y=pc)) +
  ...
  labs(title = "Percent changes in domestic hot water 2001-2005", x= "Effect", 
       y= "%",caption = "Model 3") + theme(axis.text=element_text(size=10.5),
                                           axis.title=element_text(size=12,face = "bold"),
                                           plot.title = element_text(size = 14, face = "bold"))
wabs0105<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Intensity","Electricity Efficiency","Weather"),
                     abs=c(deltot3,delPtot3,delRtot3,delTtot3,delCtot3,delHtot3,delItot3,delXtot3,delWtot3))
# check
...
sum_eff3<-delP3+delR3+delT3+delC3+delH3+delI3+delX3+delW3
db<-sum_eff3-diff3 # for debugging
wpc0509<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Intensity","Electricity Efficiency","Weather"),
                    pc=100*c(deltotpc3,dPpcbas3,dRpcbas3,dTpcbas3,dCpcbas3,dHpcbas3,dIpcbas3,dXpcbas3,dWpcbas3))
#Turn your 'treatment' column into a character vector
...
ggplot(data=wpc0509, aes(x=Effect, y=pc)) +
  ...
  labs(title = "Percent changes in domestic hot water 2005-2009", x= "Effect", 
       y= "%",caption = "Model 3") + theme(axis.text=element_text(size=10.5),
                                           axis.title=element_text(size=12,face = "bold"),
                                           plot.title = element_text(size = 14, face = "bold"))
wabs0509<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Intensity","Electricity Efficiency","Weather"),
                     abs=c(deltot3,delPtot3,delRtot3,delTtot3,delCtot3,delHtot3,delItot3,delXtot3,delWtot3))
# check
...
sum_eff3<-delP3+delR3+delT3+delC3+delH3+delI3+delX3+delW3
db<-sum_eff3-diff3 # for debugging
wpc0915<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Intensity","Electricity Efficiency","Weather"),
                    pc=100*c(deltotpc3,dPpcbas3,dRpcbas3,dTpcbas3,dCpcbas3,dHpcbas3,dIpcbas3,dXpcbas3,dWpcbas3))
#Turn your 'treatment' column into a character vector
...
ggplot(data=wpc0915, aes(x=Effect, y=pc)) +
  ...
  labs(title = "Percent changes in domestic hot water 2009-2015", x= "Effect", 
       y= "%",caption = "Model 3") + theme(axis.text=element_text(size=10.5),
                                           axis.title=element_text(size=12,face = "bold"),
                                           plot.title = element_text(size = 14, face = "bold"))
wabs0915<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Fuel","Intensity","Electricity Efficiency","Weather"),
                     abs=c(deltot3,delPtot3,delRtot3,delTtot3,delCtot3,delHtot3,delItot3,delXtot3,delWtot3))
# check
...
eff<-c("Population","Region","Type","Cohort","Fuel","Intensity","Electricity Efficiency","Weather")
wabsall<-data.frame(Effect=c("1990",eff,"1993",eff,"2001",eff,"2005",eff,"2009",eff,"2015"),
                    kBTU=c(sum(e390),wabs9093$abs[2:9],sum(e393),wabs9301$abs[2:9],sum(e301),wabs0105$abs[2:9],sum(e305),wabs0509$abs[2:9],sum(e309),wabs0915$abs[2:9],sum(e315)))
wabsall$PJ<-1.055e-9*wabsall$kBTU
wabsall$Effect3<-c("1990",eff,"2001",eff,"2009",eff,"2015",rep("NA",18))
wabsall$kBTU3<-c(sum(e390),wabs909301$abs[2:9],sum(e301),wabs010509$abs[2:9],sum(e309),wabs0915$abs[2:9],sum(e315),rep(0,18)) 
wabsall$PJ3<-1.055e-9*wabsall$kBTU3
wabsall$Effect1<-c("1990",eff,"2015",rep("NA",36))
wabsall$kBTU1<-c(sum(e390),wabs9015tot$abs[2:9],sum(e315),rep(0,36))
wabsall$PJ1<-1.055e-9*wabsall$kBTU1

write.csv(wabsall,file = paste(res, "/dhw_IDA_PE.csv",sep = ""))

# other enduse decomposition incl cohort, 1990 ######
E490<-tapply(r90$PEstOTH*r90$NWEIGHT,list(r90$TYPEHUQ,r90$DIVISION,r90$AgeCohort),sum) # energy for other uses in 1990 by type and division, agecohort
E490[is.na(E490)]<-0
N90<-tapply(r90$NWEIGHT,list(r90$TYPEHUQ,r90$DIVISION,r90$AgeCohort),sum)
N90[is.na(N90)]<-0
I490<-E490/N90
...
E490_o<-tapply(r90$PEOTH*r90$NWEIGHT,list(r90$TYPEHUQ,r90$DIVISION,r90$AgeCohort),sum) # energy for other uses in 1990 by type and division, age cohort
...
E493<-tapply(r93$PEstOTH*r93$NWEIGHT,list(r93$TYPEHUQ,r93$DIVISION,r93$AgeCohort),sum) # energy for other uses in 1993 by type and division, agecohort
E493[is.na(E493)]<-0
N93<-tapply(r93$NWEIGHT,list(r93$TYPEHUQ,r93$DIVISION,r93$AgeCohort),sum)
N93[is.na(N93)]<-0
I493<-E493/N93
...
E493_o<-tapply(r93$PEOTH*r93$NWEIGHT,list(r93$TYPEHUQ,r93$DIVISION,r93$AgeCohort),sum) # energy for other uses in 1993 by type and division, age cohort
...
E401<-tapply(r01$PEstOTH*r01$NWEIGHT,list(r01$TYPEHUQ,r01$DIVISION,r01$AgeCohort),sum) # energy for other uses in 2001 by type and division, agecohort
E401[is.na(E401)]<-0
N01<-tapply(r01$NWEIGHT,list(r01$TYPEHUQ,r01$DIVISION,r01$AgeCohort),sum)
N01[is.na(N01)]<-0
I401<-E401/N01
...
E401_o<-tapply(r01$PEOTH*r01$NWEIGHT,list(r01$TYPEHUQ,r01$DIVISION,r01$AgeCohort),sum) # energy for other uses in 2001 by type and division, age cohort
...
E405<-tapply(r05$PEstOTH*r05$NWEIGHT,list(r05$TYPEHUQ,r05$DIVISION,r05$AgeCohort),sum) # energy for other uses in 2005 by type and division, agecohort
E405[is.na(E405)]<-0
N05<-tapply(r05$NWEIGHT,list(r05$TYPEHUQ,r05$DIVISION,r05$AgeCohort),sum)
N05[is.na(N05)]<-0
I405<-E405/N05
...
E405_o<-tapply(r05$PEOTH*r05$NWEIGHT,list(r05$TYPEHUQ,r05$DIVISION,r05$AgeCohort),sum) # energy for other uses in 2005 by type and division, age cohort
...
E409<-tapply(r09$PEstOTH*r09$NWEIGHT,list(r09$TYPEHUQ,r09$DIVISION,r09$AgeCohort),sum) # energy for other uses in 2009 by type and division, agecohort
E409[is.na(E409)]<-0
N09<-tapply(r09$NWEIGHT,list(r09$TYPEHUQ,r09$DIVISION,r09$AgeCohort),sum)
N09[is.na(N09)]<-0
I409<-E409/N09
...
E409_o<-tapply(r09$PEOTH*r09$NWEIGHT,list(r09$TYPEHUQ,r09$DIVISION,r09$AgeCohort),sum) # energy for other uses in 2009 by type and division, age cohort
...
E415<-tapply(r15$PEstOTH*r15$NWEIGHT,list(r15$TYPEHUQ,r15$DIVISION,r15$AgeCohort),sum) # energy for other uses in 2015 by type and division, agecohort
E415[is.na(E415)]<-0
N15<-tapply(r15$NWEIGHT,list(r15$TYPEHUQ,r15$DIVISION,r15$AgeCohort),sum)
N15[is.na(N15)]<-0
I415<-E415/N15
...
E415_o<-tapply(r15$PEOTH*r15$NWEIGHT,list(r15$TYPEHUQ,r15$DIVISION,r15$AgeCohort),sum) # energy for other uses in 2015 by type and division, age cohort
...
opc9093<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Intensity","Electricity Efficiency"),
...
print(ggplot(data=opc9093, aes(x=Effect, y=pc)) +
        ...
        labs(title = "Percent changes in other end uses 1990-1993", x= "Effect", 
             y= "%",caption = "Model 1-3") + theme(axis.text=element_text(size=10.5),
                                                   axis.title=element_text(size=12,face = "bold"),
                                                   plot.title = element_text(size = 14, face = "bold")))
oabs9093<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Intensity","Electricity Efficiency"),
...
opc9301<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Intensity","Electricity Efficiency"),
                    pc=100*c(deltotpc,dPpcbas4,dRpcbas4,dTpcbas4,dCpcbas4,dOpcbas4,dIpcbas4,dXpcbas4))
#Turn your 'treatment' column into a character vector
...
print(ggplot(data=opc9301, aes(x=Effect, y=pc)) +
        ...
        labs(title = "Percent changes in other end uses 1993-2001", x= "Effect", 
             y= "%",caption = "Model 1-3") + theme(axis.text=element_text(size=10.5),
                                                   axis.title=element_text(size=12,face = "bold"),
                                                   plot.title = element_text(size = 14, face = "bold")))
oabs9301<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Intensity","Electricity Efficiency"),
                     abs=c(deltot4,delPtot4,delRtot4,delTtot4,delCtot4,delOtot4,delItot4,delXtot4))
...
opc0105<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Intensity","Electricity Efficiency"),
                    pc=100*c(deltotpc,dPpcbas4,dRpcbas4,dTpcbas4,dCpcbas4,dOpcbas4,dIpcbas4,dXpcbas4))
#Turn your 'treatment' column into a character vector
...
print(ggplot(data=opc0105, aes(x=Effect, y=pc)) +
        ...
        labs(title = "Percent changes in other end uses 2001-2005", x= "Effect", 
             y= "%",caption = "Model 1-3") + theme(axis.text=element_text(size=10.5),
                                                   axis.title=element_text(size=12,face = "bold"),
                                                   plot.title = element_text(size = 14, face = "bold")))
oabs0105<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Intensity","Electricity Efficiency"),
                     abs=c(deltot4,delPtot4,delRtot4,delTtot4,delCtot4,delOtot4,delItot4,delXtot4))
...
opc0509<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Intensity","Electricity Efficiency"),
                    pc=100*c(deltotpc,dPpcbas4,dRpcbas4,dTpcbas4,dCpcbas4,dOpcbas4,dIpcbas4,dXpcbas4))
#Turn your 'treatment' column into a character vector
...
print(ggplot(data=opc0509, aes(x=Effect, y=pc)) +
        ...
        labs(title = "Percent changes in other end uses 2005-2009", x= "Effect", 
             y= "%",caption = "Model 1-3") + theme(axis.text=element_text(size=10.5),
                                                   axis.title=element_text(size=12,face = "bold"),
                                                   plot.title = element_text(size = 14, face = "bold")))
oabs0509<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Intensity","Electricity Efficiency"),
                     abs=c(deltot4,delPtot4,delRtot4,delTtot4,delCtot4,delOtot4,delItot4,delXtot4))
...
opc0915<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Intensity","Electricity Efficiency"),
                    pc=100*c(deltotpc,dPpcbas4,dRpcbas4,dTpcbas4,dCpcbas4,dOpcbas4,dIpcbas4,dXpcbas4))
#Turn your 'treatment' column into a character vector
...
print(ggplot(data=opc0915, aes(x=Effect, y=pc)) +
        ...
        labs(title = "Percent changes in other end uses 2009-2015", x= "Effect", 
             y= "%",caption = "Model 1-3") + theme(axis.text=element_text(size=10.5),
                                                   axis.title=element_text(size=12,face = "bold"),
                                                   plot.title = element_text(size = 14, face = "bold")))
oabs0915<-data.frame(Effect=c("Total","Population","Region","Type","Cohort","Occupancy","Intensity","Electricity Efficiency"),
                     abs=c(deltot4,delPtot4,delRtot4,delTtot4,delCtot4,delOtot4,delItot4,delXtot4))
...
eff<-c("Population","Region","Type","Cohort","Occupancy","Intensity","Electricity Efficiency")
oabsall<-data.frame(Effect=c("1990",eff,"1993",eff,"2001",eff,"2005",eff,"2009",eff,"2015"),
                    kBTU=c(sum(e490),oabs9093$abs[2:8],sum(e493),oabs9301$abs[2:8],sum(201),oabs0105$abs[2:8],sum(e405),oabs0509$abs[2:8],sum(e409),oabs0915$abs[2:8],sum(e415)))
oabsall$PJ<-1.055e-9*oabsall$kBTU
oabsall$Effect3<-c("1990",eff,"2001",eff,"2009",eff,"2015",rep("NA",16))
oabsall$kBTU3<-c(sum(e490),oabs909301$abs[2:8],sum(e401),oabs010509$abs[2:8],sum(e409),oabs0915$abs[2:8],sum(e415),rep(0,16)) 
oabsall$PJ3<-1.055e-9*oabsall$kBTU3
oabsall$Effect1<-c("1990",eff,"2015",rep("NA",32))
oabsall$kBTU1<-c(sum(e490),oabs9015tot$abs[2:8],sum(e415),rep(0,32))
oabsall$PJ1<-1.055e-9*oabsall$kBTU1
write.csv(oabsall,file = paste(res, "/oth_IDA_PE.csv",sep = ""))


## all drivers for all enduses #####
all<-list(subset(habsall[1:12,7:9],select = c(Effect1,PJ1)),subset(cabsall[1:11,7:9],select = c(Effect1,PJ1)),subset(wabsall[1:10,7:9],select = c(Effect1,PJ1)),subset(oabsall[1:9,7:9],select = c(Effect1,PJ1))) %>%
  Reduce(function(dtf1,dtf2) full_join(dtf1,dtf2,by="Effect1"), .) # in PJ
...
write.csv(all,file = paste(res, "/all_PE.csv",sep = ""))